...
class MediaController {
	...
	async store({ request, response }) {
		...
		ffmpeg.ffprobe(`public/videos/${source}`, async (error, metadata) => {
			...
			const hashes = {}
			const cleanedFfmpegCommandArray = getFfmpegCommand(
				source,
				thumbnailRate,
				sourceAndRand
			)
			...
			ffmpeg.stderr.on('data', async (data) => {
				...
				const segmentSearchString = `Opening '${path}/stream${
					streamIterator + 1
				}.ts' for writing`
				...
				if (stringifiedData.search(segmentSearchString) > -1) {
					...
					if (dstorAddResponse.statusText === 'OK') {
						...
						if (fs.existsSync(`${path}/customStream.m3u8`)) {
							const file = fs.readFileSync(`${path}/stream.m3u8`, {
								encoding: 'utf8',
							})
			...
			ffmpeg.on('close', async (code) => {
				...
				ffprobe1.stdout.on('data', async (data) => {
					...
					try {
						...
						const fileStream = fs.readFileSync(
							`public/videos/processed/stream/${sourceAndRand}/customStream.m3u8`
						)
						const formData = new FormData()
						const formHeaders = formData.getHeaders()
						const boundaryKey = Math.random().toString(16)
